...
#   converts to millilux, and visualizes with scatter plots on Lambert Conformal projection.
# - Uses black-to-yellow colormap, millilux range 0~300
...
# Colormap: black (0) -> yellow (high)
cmap_blackyellow = LinearSegmentedColormap.from_list('blackyellow', ['black', 'yellow'])

# Grid marker size (approx 5km x 5km)
GRID_MARKER_SIZE = 100  # adjust as needed
...
def main():
    ...
    for tcol in tqdm(time_cols, desc="Processing forecasts", unit="forecast"):
        ...
        df['illum_mlux'] = df['R_light'] * df[tcol].map(CLOUD_FACTOR).fillna(DEFAULT_FACTOR) * 1000.0
        ...
        sc = ax.scatter(
            ...
            cmap=cmap_blackyellow,
            vmin=0, vmax=300
        )
        cbar = plt.colorbar(sc, ax=ax, orientation='vertical', pad=0.02)
        cbar.set_label('Illuminance (millilux)')
        ...
        ax.set_title(f"Illuminance (mlux) at {tcol}")

        # Save figure
        out_fname = os.path.join(os.path.dirname(csv_path), f"illum_{tcol}.png")
